...
\subsection{Matrix: Adaptação Federada com Olm e Megolm}

O Matrix difere do modelo centralizado do Signal ao propor um padrão aberto para comunicação federada, onde servidores distintos interagem em tempo real. A segurança nesse ambiente distribuído é gerida por dois algoritmos complementares, auditados formalmente por \cite{ungerman2018security}: o \textbf{Olm} e o \textbf{Megolm}.

O \textbf{Olm} é uma implementação estrita do \textit{Double Ratchet} descrita anteriormente, utilizada para estabelecer canais seguros 1:1 entre dispositivos. No entanto, replicar sessões \textit{pairwise} (par-a-par) em grupos massivos gera uma complexidade de mensagens $O(N^2)$, tornando o protocolo original do Signal inviável para salas com milhares de participantes.

Para resolver o problema da escalabilidade, o Matrix introduziu o \textbf{Megolm}. Neste mecanismo, cada participante inicializa uma única "sessão de saída" (\textit{outbound session}) baseada em um \textit{ratchet} unidirecional. A semente inicial desta catraca é compartilhada com os demais membros do grupo através dos canais seguros Olm.

A partir desse estabelecimento, o remetente deriva novas chaves localmente para cada mensagem enviada, reduzindo a complexidade de transmissão para $O(N)$. A compensação técnica necessária para essa eficiência é o enfraquecimento da \textit{Segurança Pós-Comprometimento}: ao contrário do Signal, o Megolm não executa a "auto-cura" (rotação via Diffie-Hellman) a cada mensagem. A recuperação do sigilo após um comprometimento depende da rotação periódica da sessão completa, configurável pelo cliente ou servidor \cite{matrix_spec}.